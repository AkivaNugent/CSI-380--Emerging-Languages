...
extension BASICPlayer {
    ...
    public mutating func interpret(statement: Statement) throws {
        ...
        case let varSet as VarSet:
            //evalute the varSet expression
            //add to variable tavle
            //increment the index
            variableTable[varSet.name.lowercased()] = evaluate(expression: varSet.value)
            statementIndex += 1
            
        case let ifStatement as IfStatement:
            //check if the bool passes
            //then try to interperate the then
            //increment
        ...
        case let goToColl as GoToCall:
            //jump to index given
            //dont increment because were jumping
            statementIndex = findLineIndex(lineNumber: goToColl.gotoLine)!
            
            
        case let goSubCall as GoSubCall:
            //add lext line to substack for return
            //goto
            subroutineStack.push(statementIndex + 1)
            try interpret(statement: GoToCall(gotoLine: goSubCall.gotoLine, line: goSubCall.line, range: goSubCall.range))
            
        case let returnStatement as ReturnStatement:
            if let returnTo = subroutineStack.pop() {
                statementIndex = returnTo
            } else {
                statementIndex += 1
            }
